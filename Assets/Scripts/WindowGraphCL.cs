...
using System;
public class WindowGraphCL : MonoBehaviour
{
    ...
    private void Awake() {
    	...
    	ShowGraph(UIManager.currentList, -1,(int _i) => UIManager.loadList[_i].ToString().Substring(0,5), (float _f) => _f.ToString().Substring(0,5)); // custom labels
    }
    ...
    private void ShowGraph(List<float> valueList, int maxVisibleValueAmount=-1,Func<int,string> getAxisXLabel=null, Func<float,string> getAxisYLabel=null) {
    	...
    	for(int i=Mathf.Max(0,UIManager.currentList.Count-maxVisibleValueAmount);i<valueList.Count;i++){
    	...
    	for(int i=Mathf.Max(UIManager.loadList.Count-maxVisibleValueAmount,0);i<UIManager.loadList.Count;i++){
